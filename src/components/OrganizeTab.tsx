...
export default function OrganizeTab() {
  ...
  const handleDeleteEvent = async (eventId: string) => {
    ...
    try {
      const { success, error } = await deleteEvent(eventId, user.id);
          ...
          {userEvents.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {userEvents.map((event) => (
                <div key={event.id} className="glass-effect p-6 rounded-2xl hover:bg-white/5 transition-all space-y-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-white mb-2">{event.title}</h3>
                      {event.organizer_specification && (
                        <p className="text-sm text-purple-300 mb-2">by {event.organizer_specification}</p>
                      )}
                  ...
                  <div className="flex gap-2">
                    {!event.is_published && (
                      <button
                        ...
                        disabled={isLoading}
                        className="flex-1 px-4 py-2 bg-gradient-to-r from-rose-500 to-purple-600 text-white rounded-lg font-medium hover:shadow-lg transition-all text-sm flex items-center justify-center space-x-2 disabled:opacity-50"
                      >
                    ...
                    <button
                      ...
                      disabled={isLoading}
                      className="flex-1 px-4 py-2 glass-effect text-gray-300 rounded-lg hover:text-white transition-all text-sm flex items-center justify-center space-x-2"
                    >
                    ...
                    <button
                      onClick={() => {
                        setSelectedEventForBooking(event.id);
                        setActiveView('browse-services');
                      }}
                      className="flex-1 px-4 py-2 glass-effect text-gray-300 rounded-lg hover:text-white transition-all text-sm"
                    >
                      Add Services
                    </button>
                    <button
                      ...
                      disabled={isLoading}
                      className="px-4 py-2 glass-effect text-rose-400 hover:text-rose-300 transition-all text-sm disabled:opacity-50"
