...
export default function JoinTab({
  ...
  // Convert database events to Firebase Event type
  const convertDBEventToFirebaseEvent = (dbEvent: DBEvent): Event => {
    return {
      id: dbEvent.id,
      title: dbEvent.title,
      category: dbEvent.category as any,
      date: dbEvent.event_date,
      time: dbEvent.event_time ? `${dbEvent.event_time} EAT` : 'TBA',
      location: dbEvent.location,
      organizer: dbEvent.organizer_specification || dbEvent.organizer_name,
      organizer_id: dbEvent.organizer_id,
      image_url: dbEvent.image_url || 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=800',
      description: dbEvent.description || '',
      price: dbEvent.price || 0,
      rating: dbEvent.rating || 0,
      features: dbEvent.features || [],
      speakers: dbEvent.attractions || [],
      status: dbEvent.status as any,
      is_livestream: dbEvent.is_livestream || false,
      livestream_url: dbEvent.livestream_url || null,
      created_at: dbEvent.created_at,
      updated_at: dbEvent.updated_at,
    };
  };

  const events: Event[] = publishedEvents.map(convertDBEventToFirebaseEvent);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredEvents.map((event) => (
            <EventCard
              ...
              onEdit={(eventForEdit) => {
                const dbEventTyped = publishedEvents.find(e => e.id === eventForEdit.id);
                if (dbEventTyped) setEditingEvent(dbEventTyped);
              }}
            />
          ))}
        </div>
